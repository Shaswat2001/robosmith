 ...
 
{
  ...
  "safety_constraints": [
    {"description": "Do not exceed 10N force on the object"}
  ]
}
 
Rules:
- robot_type: infer from the description. Arms/grippers = "arm". Legs = "quadruped"/"biped".
  Hands with fingers = "dexterous_hand". Wheeled = "mobile_base". Unknown = "custom".
...
- algorithm: default "auto". Use "sac" if the task seems to need sample efficiency
  (complex manipulation). Use "ppo" for locomotion.
- success_criteria: always include success_rate >= 0.8 as baseline.
  Add task-specific ones if clear from description.
